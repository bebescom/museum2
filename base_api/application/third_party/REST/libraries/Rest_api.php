...
//        $this->sh = curl_share_init();
    ...
    function _add($uri, $data = null)
    {
        ...
        if (is_array($_FILES) && count($_FILES) > 0) {
            ...
            $new_path=$_SERVER['DOCUMENT_ROOT'].'/uploads/tmp/';
            if (!is_dir($new_path)) {
                mkdir($new_path, 0766, true);
            }
            foreach ($_FILES as $key => $file) {
                $file['rest'] = config_item('local_api');
                $tmp_name=$file['tmp_name'];
                $file_name=md5_file($file['tmp_name']);
                $tmp_file = $file['tmp_name'] = str_replace('/', DIRECTORY_SEPARATOR, $new_path . $file_name);
        ...
        $options = array(
            ...
            CURLOPT_CUSTOMREQUEST => strtoupper($method), // GET POST PUT PATCH DELETE HEAD OPTIONS
            CURLOPT_HTTPHEADER => array(
                config_item('rest_token_param_name') . ':' . $token_row['token'],
                'X-HTTP-Method-Override: ' . strtoupper ($method),
        ...
        if($this->_ci->input->raw_input_stream){
            $options[CURLOPT_HTTPHEADER][]='Content-Type:application/json';//.$_SERVER['CONTENT_TYPE'];
