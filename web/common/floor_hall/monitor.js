define('common/floor_hall/monitor', function(require, exports, module) {
//
// 
// 
// 
...
var vm,
    envBar = require('common/floor_hall/components/envBar/envBar.js');//引入组件
exports.init = function (env_no) {
    ...
    $.get('/2.2.05_P001/base_api/env/environments/hall_standards/' + env_no+'', function (data) {
        gauge_chart(data.rate);
    }, 'json');

    $.get('/2.2.05_P001/base_api/env/environments/hall_params/' + env_no+'', function (data) {
        if (data.total == 0) {
            return;
        }
        radar_chart(data);
    ...
    $.get('/2.2.05_P001/base_api/env/environments/equip_status/' + env_no+'', function (data) {
        vm.$set('normal_total', data['正常'] * 1);
        vm.$set('abnormal_total', data['异常'] * 1);
        vm.$set('sensor_total', vm.normal_total + vm.abnormal_total);
    }, 'json');

    $.get('/2.2.05_P001/base_api/env/environments/floor_sensors/' + env_no+'', function (data) {
...
function get_poles(param,time) {
    var key=(time)?time:'',
        me=this;
    $.get('/2.2.05_P001/base_api/env/environments/floor_poles/' + vm.env_no + '/' + param+''+'?time='+key, function (data) {
        //console.log(data);
