define('setting/env', function(require, exports, module) {
var vm, treeObj, time;
var type_lists = [
...
function add_env(treeNode) {
    ...
    $.get('/2.2.05_P001/base_api/base/envs/add/' + parent_env_no+'', function (json) {
...
function delete_env(treeNode) {
    ...
    layer.confirm('确认删除', {btn: ['删除', '取消']}, function (index) {
        $.post('/2.2.05_P001/base_api/base/envs/delete/' + treeNode.env_no+'', function (json) {
...
function save_env() {
    ...
    clearTimeout(time);
    var url = '/2.2.05_P001/base_api/base/envs/edit/' + env.env_no+'';
    if (env.new_node) {
        url = '/2.2.05_P001/base_api/base/envs/add/' + env.env_no+'';
    }
    if (env._is_root) {
        url = '/2.2.05_P001/base_api/base/setting/bg';
    }
...
function load_pic(env_no) {
    ...
    $.get('/2.2.05_P001/base_api/base/setting/pic/' + env_no+'', function (data) {
        if (!data || data == '[]') {
            return;
        }
...
function add_env_pic(url) {
    ...
    $.post('/2.2.05_P001/base_api/base/setting/pic/' + vm.env.env_no+'', {url: url}, function (data) {
        load_pic();
    }, 'json');
...
function del_pic(id) {
    layer.confirm('确认删除', {btn: ['删除', '取消']}, function (index) {
        $.post('/2.2.05_P001/base_api/base/setting/pic_del/' + id+'', function (data) {
            load_pic();
        }, 'json');
        layer.close(index);
    });
...
function set_home_pic(id) {
    $.post('/2.2.05_P001/base_api/base/setting/pic_home/' + id+'', function (data) {
        load_pic();
    }, 'json');
}
});