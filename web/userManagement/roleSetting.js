define('userManagement/roleSetting', function(require, exports, module) {
...
module.exports = Vue.extend({
   ...
   ready:function(){
      var _this = this;
      $.get('/2.2.05_P001/base_api/base/envs',function(data){
   ...
   methods:{
      configData:function(){//默认配置信息数据
         var _this = this;
         $.get('/2.2.05_P001/base_api/base/config',function(data){
      ...
      getPermissionData:function(){//获取权限树数据
         ...
         $.get('/2.2.05_P001/base_api/base/permissions/tab_tree',function(data){
            console.log(data)
            if (data){
      ...
      getRolesData:function(){//获取角色列表数据
         $.get('/2.2.05_P001/base_api/base/users/info/'+this.singleCurrentRoleId,function(data){//获取当前用户角色列表
            _this.currentUserRoleList = data.role_names.split(',');
         });
         var _this = this;
         setTimeout(function(){
            $.get('/2.2.05_P001/base_api/base/roles',function(data){
               ...
               $.get('/2.2.05_P001/base_api/base/roles/info/'+_this.currentRoleId,function(data){//每次切换角色时获取对应角色的数据
      ...
      addSaveRole:function(){//添加保存按钮
         ...
         if (!this.checkTongB&&!this.checkTongEmpty&&this.inputRoleName){
            $.post('/2.2.05_P001/base_api/base/roles/add',{name:this.inputRoleName},function(data){
      ...
      saveRole:function(){//
         ...
         this.$Modal.confirm({
            ...
            content: '是否改变角色权限',
            onOk:function(){
               $.post('/2.2.05_P001/base_api/base/roles/edit/' + _this.currentRoleId,{name:_this.currentRole,permissions:sendPermissions,data_scope:_this.DataPermissions},function(data){
      ...
      removeRole:function(val){//删除角色（右上角）
         ...
         $.each(this.rolesDataCopy,function(index,elem){
            if (val === index){
               $.post('/2.2.05_P001/base_api/base/roles/delete/'+ elem.id+'',function(data){
      ...
      changeRole:function(val){//切换角色
         ...
         ///获取环境树/
         $.get('/2.2.05_P001/base_api/base/roles/info/'+this.rolesDataCopy[val].id,function(data){//每次切换角色时获取对应角色的数据
            _this.changeRoleEnvDataPermissions = data.data_scope;
            if (_this.changeRoleEnvDataPermissions){
               _this.changeRoleEnvDataPermissions =  _this.changeRoleEnvDataPermissions.split(',');
               _this.dataPermissionsSelectStatus();//对数据权限进行判断，选择其对应状态
