define('equipManage/equip_list', function(require, exports, module) {
var vm;
...
function filter_data() {
    ...
    $.get('/2.2.05_P001/base_api/env/equipments/count/status_count', function (data) {
        var row = {
            type: 'status', key: 'status', name: '状态', data: data
        };
        search_data.right_list.push(row);

        $.get('/2.2.05_P001/base_api/env/equipments/count/type_count', function (data) {
            var row = {
...
function getEquipList(search_data) {
    ...
    // console.log(param);
    $.post('/2.2.05_P001/base_api/env/equipments/manage/equip_list', param, function (data) {
...
function edit_remark(index) {
    ...
    layer.prompt({title: '输入备注信息', value: equip.remark, formType: 2}, function (text, index) {
        layer.close(index);
        if (text) {
            $.post('/2.2.05_P001/base_api/env/equipments/manage/edit/', {equip_no: equip.equip_no, remark: text}, function (data) {
...
function deleteEquip() {
    ...
    layer.confirm('确认删除以下' + count + '个设备?<br/>' + nos.replace(/,/g, '<br/>') + '', {btn: ['确认删除', '取消']}, function (index) {
        $.post('/2.2.05_P001/base_api/env/equipments/manage/delete/' + nos, function (data) {
            vm.$Message.info(data.msg);
