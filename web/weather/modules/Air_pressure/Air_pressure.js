define('weather/modules/Air_pressure/Air_pressure', function(require, exports, module) {
//获取参数单位名称
var param_unit_name = window.web_config.param_unit_name.weather;

module.exports={
	template:'#Air_pressure',
	data:function(){
		return {
			default_option:null,
			myChart:null
	...
	ready:function(){
		...
		this.myChart=echarts.init($('#air_pressure')[0]);
	...
	events:{
		Air_pressure:function(data,from){
			if(from==='local'){
				this.local(data);
			}else{
				this.online(data);
