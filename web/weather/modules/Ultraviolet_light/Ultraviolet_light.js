define('weather/modules/Ultraviolet_light/Ultraviolet_light', function(require, exports, module) {
//获取参数单位名称
var param_unit_name = window.web_config.param_unit_name.weather;

module.exports={
	template:'#Ultraviolet_light',
	data:function(){
		return {
			default_option:null,
			myChart:null
	...
	ready:function(){
		...
		this.myChart=echarts.init($('#ultraviolet_light')[0]);
	...
	events:{
		Ultraviolet_light:function(data,from){
			if(from==='local'){
				this.local(data);
			}else{
				this.online(data);
