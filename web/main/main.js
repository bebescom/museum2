...
function show_main() {
    ...
    $('#content').load('main.html?date=' + new Date() * 1, function () {
        getSvg();
        require('main/footer').init();
        require('main/right').init();
...
router({
    when: {
        ...
        '/floor/:env_no/:floor_no': function (env_no, floor_no) {
            ...
            require('main/floor').open(env_no, floor_no);
        }
    }
});
...
function getSvg() {
    ...
    $.get('/2.2.05_P001/base_api/base/envs', {type: '楼栋'}, function (data) {
        if (data.total == 0) {
            return;
        }
        var svg = draw({
            ...
            mouseup: function (env_no) {
                require('main/floor').open(env_no);
            },
            draw: true,
