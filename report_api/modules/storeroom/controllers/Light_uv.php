 ...
 * Time: 17:58
 */
class Light_uv extends MY_Controller
{
...
//        $this->response($all_data);
        if($all_data){
            foreach ($all_data as $sht_name=>$ads) {
                if($ads){
                    foreach($ads as $ad_param=>$ad){
                        $contents[$sht_name.$params[$ad_param]['name'].'描述'] = desc_judge($ad,$ad_param);
                    }
                }
            }
        }
                    ...
                    foreach ($tb_datas as $tb_param=>$tb_data) {
                        if($tb_data){
                            foreach ($tb_data as $tb) {
                                ...
                                $tb['sheet_name'] = $sheet_name.'小环境'.$params[$pd_param]['name']."数据描述性统计量";
