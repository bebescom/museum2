 ...
 * Time: 13:55
 */
class Temperature extends MY_Controller
{
...
//                    }
                   ...
                   if(isset($data_sensors['rows'])&&$data_sensors['rows']){
                       foreach ($data_sensors['rows'] as $ds) {
                           ...
                           //箱线图数据和极值数据
                           if(isset($ds['temperature'])&&$ds['temperature'] != "NaN"&&$ds['temperature'] != ""&&$ds['temperature'] != null&&data_filter($ds['temperature'],"temperature")){
                               if(data_filter($ds['temperature'],"temperature")){
                                   if(!isset($pdata[$env_name])){
                                       $pdata[$env_name] = array();
                                   }
                                   $pdata[$env_name][] = $ds['temperature']+0;
                                   if(!isset($all_data['all'][$env_type])){
                                       $all_data['all'][$env_type] = array();
                                   }
                                   $all_data['all'][$env_type][] = $ds['temperature']+0;
                                   ...
                                   if(!isset($max[$env_type]['value'])){
                                       $max[$env_type]['value'] = $ds['temperature'];
                                       $max[$env_type]['name'] = $name;
                                   }else{
                                       if($ds['temperature'] > $max[$env_type]['value']){
                                           $max[$env_type]['value'] = $ds['temperature'];
                                           $max[$env_type]['name'] = $name;
                                       }
                                   }
                                   if(!isset($min[$env_type]['value'])){
                                       $min[$env_type]['value'] = $ds['temperature'];
                                       $min[$env_type]['name'] = $name;
                                   }else{
                                       if($ds['temperature'] < $min[$env_type]['value']){
                                           $min[$env_type]['value'] = $ds['temperature'];
                                           $min[$env_type]['name'] = $name;
                                       }
                                   }
        ...
        if($max){
            foreach($max as $ty=>$val){
                $data[$ty.'温度最大值'] = isset($val['value'])?$val['value']:"";
                $data[$ty.'温度最大值环境'] = isset($val['name'])?$val['name']:"";
            }
        }
        if($min){
            foreach($min as $ty=>$val){
               if($val){
                   $data[$ty.'温度最小值'] = isset($val['value'])?$val['value']:"";
                   $data[$ty.'温度最小值环境'] = isset($val['name'])?$val['name']:"";
               }
            }
        }
        if($pole_diff){
            foreach($pole_diff as $ty=>$val){
                if($val){
                    $data[$ty.'温度全距最小值'] = round(min($val),2);
                    $data[$ty.'温度全距最大值'] = round(max($val),2);
                }
            }
        }
        if($std_array){
            foreach($std_array as $ty=>$val){
               if($val){
                   $data[$ty.'温度标准差最小值'] = round(min($val),2);
                   $data[$ty.'温度标准差最大值'] = round(max($val),2);
               }
            }
        }
        if($avg){
            foreach($avg as $ty=>$val){
              if($val){
                  $data[$ty.'温度均值最小值'] = round(min($val),2);
                  $data[$ty.'温度均值最大值'] = round(max($val),2);
              }
            }
        }
        //描述性判定
        if(isset($box_plot)){
            foreach($box_plot as $ty=>$val){
                $data[$ty.'温度描述'] = desc_judge($val,"temperature");
                if(isset($all_data['wave'][$ty])){
                    $data[$ty.'温度描述'] .= desc_judge($all_data['wave'][$ty],"temperature",'全距');
                }
            }
        }
//$this->response($data);
        ...
        if($box_plot){
            foreach($box_plot as $type=>$bp_data){
                $image_key = "各".$type."温度箱型图";
//                if($type == "展厅"){
//                    $image_key = "展厅温度箱型图";
//                }else if($type == "库房"){
//                    $image_key = "库房温度箱型图";
//                }else{
//                    $image_key = "考古整理楼温度箱型图";
//                }
                $bp_image = array(
                    'image_type'=>'box_plot',
                    'image_key'=>$image_key,
