    ...
    public function _remap($object_called, $arguments)
    {
        ...
        if (method_exists($this, $object_called . '_' . $this->_method . '_nologin')) {//不需要验证token登录的,无_user
            ...
            if (method_exists($this, 'index_' . $this->_method . '_nologin')) {//不需要验证token登录的index
                ...
                $check_token_user = false;
            } else if (method_exists($this, 'index_' . $this->_method)) {//需要验证token登录的index
...
//        var_dump($controller_method);
        ...
        if ($check_token_user&&config_item('rest_token_enable') and (!$this->_token || strlen($this->_token) < 40)) {
    ...
    public function response($data = null, $http_code = null, $continue = false)
    {
        ...
        if ($callback = $this->get('callback')) {
            ...
            $content_type = 'js';
            if($iframe=='1'){
                $output='<script type="text/javascript">window.parent.'.$output.'</script>';
                $content_type = 'html';
            }
    ...
    protected function _detect_api_token()
    {
        ...
        // }else{
            ...
            $token_row = $this->rest_token->create_token();
            $this->_token=$token_row['token'];
        // }
    }
    ...
    {
        if (!config_item('rest_token_enable')||!$this->_token) {
            return;
        }
        $this->load->library('rest_token');
