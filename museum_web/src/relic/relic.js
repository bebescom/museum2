...
$.post(API('/base/follows'),{no:relic_no},function(data){
    if (data.total) {
...
var vm = exports.vm = new Vue({
    ...
    created:function(){
        ...
        $.get(API('/env/common/threshold/')+relic_no+'/relic',function(data){
            if(data.error){
            ...
            if(data[relic_no]){
                ...
                // console.log(API('/env/common/threshold/')+relic_no+'/relic',strData);
                This.threshold.now=JSON.parse(strData);
            }
        });
        $.get(API('/relic/relics/images/pic/'+relic_no),function(data){
            if(data.error){
                This.$Message.error(data.error);
                return;
            }
    ...
    methods: {
        ...
        getParamsList:function(env_no,relic_no){
            ...
            $.get(API('/env/environments/cabinet/new_data/'+env_no),function(data){
                if(data.error){
        ...
        getParamsLinesData:function(env_no,relic_no){
            ...
            $.get(API('/env/environments/cabinet/param_lines/'+env_no),{param:this.checkList.join(','),time:this.filterTime.join(','),relic_no:relic_no},function(data){
                if(me.lineChart){
        ...
        getStandardData:function(){
            ...
            // console.log('请求达标率数据');
            $.get(API('/env/environments/cabinet/standard/') + this.env_no,{time:this.filterTime.join(',')},function(data){
                if(me.gaugeChart){
        ...
        getRadarData:function(){
            ...
            $.get(API('/env/environments/cabinet/param_standard/') + this.env_no,{time:this.filterTime.join(',')},function(data){
                // console.log(data);
                if(me.radarChart){
        ...
        getStableData:function(){
            $.get(API('/env/environments/cabinet/stable/' + this.env_no),{time:this.filterTime.join(',')},function (data) {
        ...
        changeLockStatus:function(relic_no){
            ...
            if(this.thresholdLock==1){
                $.post(API('/env/common/threshold/lock/'+relic_no+'/0'),function(data){
                    if(data.result){
                        me.thresholdLock=0;
            ...
            //当前未锁定
                ...
                $.post(API('/env/common/threshold/lock/'+relic_no+'/1'),function(data){
                    if(data.result){
                        me.thresholdLock=1;
        ...
        postNewData:function(obj){
            // console.log(obj);
            var This=this;
            $.post(API('/env/common/threshold/add_edit'),obj,function(data){
                if(!data.result){
        ...
        resetName:function(type,e){
            ...
            $.post(API('/relic/relics/manage/edit'),this.relicView,function(data){
                if(data.error){
                    This.$Message.error(data.error);
                    return;
                }
        ...
        changeRelicImage:function(data){//更换图片
            ...
            if(this.moreImages.length > 0){
                $.each(this.moreImages,function(index,elem){
                    $.post(API('/relic/relics/images/pic_del/'+elem),function(data){
                        console.log(data);
                    })
                })
            }
            setTimeout(function(){
                $.post(API('/relic/relics/images/pic/'+relic_no),{url:data.url},function(data){
					console.log(data);
        ...
        deleteRelic:function(){//删除当前文物
            ...
            this.$Modal.confirm({
                ...
                onOk: function() {
                    $.post(API('/relic/relics/relics/delete/'+ relic_no),function(data){
...
$relicName.click(function(){
    if (key) {
        $.post(API('/base/follows/delete/'),{no:relic_no},function(data){
            if (data.msg!="删除成功") {
        ...
        $.post(API('/base/follows/add'),{name: name, no: relic_no,type:'relic'},function(data){
            if (data.msg!="添加成功!") {
                // console.error(data.error);
                vm.$Message.error(data.error);
                return;
...
$.get(API('/relic/relics/relic_overview/' + relic_no), function (data) {
    if(data.error){
        vm.$Message.error(data.error);
        return;
    }
    ...
    if (parent_env_no != '') {
        ...
        vm.getStableData();
        //监测设备-start
        $.get(API("/env/environments/cabinet/equip_lists/" + parent_env_no + "/sensor"), function (data) {
            vm.$set('sensor_list', data.rows);
        });

        //调控设备-start
        $.get(API("/env/environments/cabinet/equip_lists/" + parent_env_no + "/controller"), function (data) {
