...
$.post(API('/base/follows'),{no:equip_no},function(data){
    if (data.total) {
...
$relicName.click(function(){
    if (key) {
        $.post(API('/base/follows/delete/'),{no:equip_no},function(data){
            if (data.msg!="删除成功") {
        ...
        $.post(API('/base/follows/add'),{name: name, no: equip_no,type:'equip'},function(data){
            if (data.msg!="添加成功!") {
                console.log(data.error);
                return;
            }
...
$.get(API('/env/equipments/overviews/' + equip_no), function (data) {
    name=data.name;
    vm.$set('equip', data);
...
$.get(API('/env/equipments/equip_parameters/sensor'), function (data) {
    var param_unit = {};
...
function get_data_list(timeL) {
    ...
    $.get(API('/env/equipments/data_lists/' + equip_no + '/' + timeL), function (data) {
		if (data.total == 0) {
