// @require css/Environmental_levels.css
...
module.exports={
	...
	ready:function(){
		...
		$.post(API('/base/users/active/alert_activity'),{alert_type:"env"},function(res){
			if(!res.result){
	...
	methods:{
		...
		getData:function(filterObj){
			...
			$.get(API('/env/monitor/alerts/alerts_lists'),post_data,function(data){
        ...
        goNavDetail:function(nav,alertTime,param){//获取报警时间，并处理
			...
			if(nav){
				if(nav[nav.length-1]){
					...
					$.get(API('/env/monitor/line'),{env_no: alertNo,alert_time:alertTimeA,alert_param:param,time:time},function(data){
		...
		'clear-police':{
			...
			methods:{
				...
				clearAlert:function(){
					...
					//console.log(time)
					$.post(API('/env/monitor/alerts/clear/')+this.clearData.id,{clear_time:time,remark:this.text},function(data){
						This.key=true;
						if(data.result!=true){
							//alert('处理失败');
