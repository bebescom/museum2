...
module.exports={
	...
	methods:{
		...
		get_equipment_table_data:function(env_no){//获取列表数据
			...
			$.get(API('/env/monitor/equip?env_no='+env_str+'&no_name='+this.search_equipment_name+'&equip_type='+(this.filter_list.equip_type.join(','))+'&status='+(this.filter_list.status.join(','))+'&limit='+this.limit+'&page='+this.curr_page+'&index='+this.equipment_table_data_index),function(data){
        ...
        get_equipment_status:function(){
			var me = this;
            $.get(API('/env/equipments/count/status_count'),function(res){
                // console.log(res);
                if(res){
                	me.equip_status = res;
				}
