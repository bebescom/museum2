    ...
    error: function (XMLHttpRequest, textStatus, errorThrown) {
        ...
        if (XMLHttpRequest && XMLHttpRequest.responseJSON && XMLHttpRequest.responseJSON.error === '没有登录') {
            vm.$Message.error(XMLHttpRequest.responseJSON.error + ',即将返回登录界面...', 3, function () {//当没有登录时,提示并跳转到首页
                store.session.clear();
                window.location.href = __uri('/login') + '?url=' + location.href;
            });
...
var vm = exports.vm = window.header_vm = new Vue({
    ...
    ready: function () {
        var This = this;
        $.get(API('/env/monitor/alerts/alert_unknown'), function (data) {//之前未清除消息
    ...
    methods: {
        ...
        warningList: function () {
            ...
            var This = this;
            $.post(API('/base/users/active/alert_activity'), {alert_type: 'env'}, function (data) {//更新消息的查看状态
                console.log(data)
            })
        },
        warningNum: function () {
            ...
            $.get(API('/env/monitor/alerts/alert_unknown'), {index: this.warn_times}, function (data) {//之前未清除消息
