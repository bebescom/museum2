...
function getFloorList(parent, floor_no) {
    ...
    $.get(API('/base/envs/tree/' + parent.env_no), function (data) {
...
function show_info() {
    ...
    $.get(API('/base/envs'), {parent_env_no: row.env_no}, function (data) {
        draw_hall(row.env_no, data);
    }, 'json');

	var env_chart=showLoad($('#env_basic')[0]);
    $.get(API('/env/environments/floor/floor_halls/' + vm.floor_env_no), function (data) {
    	env_chart.hideLoading();
        env_basic(data,env_chart);
    }, 'json');

	var relic_chart=showLoad($('#relic_basic')[0]);
    $.get(API('/relic/relics/count/relics_level/' + vm.floor_env_no), function (data) {
    	console.log(data)
    	var num=0;
    ...
    $.get(API('/env/environments/floor/floor_equips/' + vm.floor_env_no), function (data) {
