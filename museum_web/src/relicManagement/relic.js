...
module.exports={
	...
	methods:{
		relicsOverviewSheet:function(){
			// $.get(API("/relic/export/relics"),function(data){
			// 	console.log('test',data);
			// 	if(data&&data.result){
			// 		console.log(data);
			// 		//window.location.href = data.url;
			// 		var target = API('')+data.url.slice(1);
			// 		window.open(target);
			// 	}
			// });
            var _this = this;
		...
		getEcharts:function(){
			...
			$.get(API('/relic/relics/count/category_count'),this.filter,function(data){//获取材质饼图数据
				This.$broadcast('relic_type',data);
				key1=true;
				This.echarts_key=key1&&key2;
			});
			$.get(API('/relic/relics/count/age_count'),this.filter,function(data){//获取年代饼图数据
		...
		getRelic:function(filterObj,resetId){
			...
			$.get(API('/relic/relics/manage/relic_list'),filter,function(data){
