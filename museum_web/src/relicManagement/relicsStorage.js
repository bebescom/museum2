...
// @require ./css/relicsStorage.css
// @require ../equipManagement/css/equip_content.css
...
module.exports = Vue.extend({
    ...
    computed:{
        ...
        //     return API('/relicBox/relicBoxTemplate.xls')+'&access_token='+window.web_config.token;
        // },
        uploadUrl:function(){//批量上传Excel
            return API("/relic/import/relics") + '?access_token='+window.web_config.token;

            // return API('/relicBox/importFromExcel')+'&access_token='+window.web_config.token;
        },
        uploadPicUrl:function(){//上传图片
            return API("/relic/import/images") + '?access_token='+window.web_config.token;
        }
    },
    methods:{
        // 文物批量导入功能
        ...
        downloadTemplate:function(){
            ...
            // console.log('下载模版');
            $.get(API('/base/download'),config,function(res){
                if(!res.result){
                    ...
                    if(typeof res.url === "string"){
                        ...
                        //拼接模版下载地址
                        var target = API('')+res.url.slice(1);
                        window.open(target);
                    }
        ...
        storage_ok:function(){
            ...
            if(this.selRelics&&this.selRelics.length!==0){
                if(!this.status){
                    ...
                    $.ajax({
                        type:"POST",
                        url:API('/relics/instockRecord/instock')+'?requestType=pc&access_token='+window.web_config.token,
        ...
        getRelic:function(filterObj){
            ...
            $.ajax({
                type:"GET",
                url:API('/relics/instockRecord/relic/list/'+this.page+'/'+this.limit),
                data:this.filter,
                error:function(XMLHttpRequest, textStatus, errorThrown){
        ...
        'add-relic':{
            ...
            methods:{
                get_filter_conditions:function(){
                    var me = this;
                    $.get(API('/relic/relics/count/filter_conditions'),function(res){
                        // console.log(res);
                        if(res){
                ...
                continueOver:function(key){
                    ...
                    $.post(API('/relic/relics/relics/add'),this.addData,function(data){
                        // console.log(data);
                ...
                editRangeFun:function(rangeData,no,type){//添加文物时修改阈值函数
                    ...
                    $.post(API('/env/common/threshold/add_edit'),rangeData,function(data){
                        console.log(data);
                    })
                },
                addRelicImage:function(no){
                    $.post(API('/relic/relics/images/pic/'+no),{url:this.uploadImageUrl},function(data){
                ...
                meterial_change:function(data){//材质选择
                    var _this = this;
                    $.get(API('/relic/common/relic_threshold/'),{type:this.addData.material},function(data){
