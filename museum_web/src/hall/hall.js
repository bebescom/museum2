...
$.get(API('/base/envs/info/' + env_no), function (data) {
    ...
    $.get(API('/env/environments/floor/hot_area/'+env_no), function (data) {//勾画热区
        // console.log(data);
        get_svg(data);
    }, 'json');
}, 'json');
...
function show_cabinet(env_no, e) {
    ...
    $('body').append($box);
    $.get(API('/env/environments/env_detail/' + env_no), function (data) {
...
function cabinet_init(env_no, $box, data) {//弹窗
    ...
    function insertBtn($box, myChart) {
        ...
        $.get(API('/env/environments/cabinet/param_lines/' + env_no), function (data) {
            console.log(data);
...
function load_data(count_time) {//加载数据
    ...
    $.get(API('/env/general/field'),{env_no:env_no,times:newIntervalTime,count_time:count_time},function(json){
