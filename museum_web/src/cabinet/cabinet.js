...
var env_no = router.get('env_no'),
    ...
    name='';
//
//$.post(API('/base/follows'),{no:env_no},function(data){
//  if (data.total) {
...
//  if (key) {
//      $.post(API('/base/follows/delete/'),{no:env_no},function(data){
//          if (data.msg!="删除成功") {
...
//      });
//  }else{
//      $.post(API('/base/follows/add'),{name: name, no: env_no,type:'cabinet'},function(data){
//          if (data.msg!="添加成功!") {
//              console.log(data.error);
//              return;
//          }
    ...
    methods: {
        ...
        edit_threshold:function(e){//编辑阈值
            ...
            if(!this.in_edit){//不在编辑中,则进入编辑
                ...
                if(!this.data_error){
                    $.post(API('/env/common/threshold/add_edit'),result_obj,function(data){
                        if(data.result){
                            me.in_edit = false;
...
$.get(API('/base/envs/info/' + env_no), function (data) {
    name=data.name;
    vm.$set('cabinet', data);
    document.title = data.name||'' +'-' + document.title;
}, 'json');
$.get(API('/env/common/threshold/'+env_no+'/env'), function (data) {
    if (data == '' || data == '[]') {
        vm.isEmpty=true;
        return;
    }
...
//监测设备-start
//$.get(API("/env/environments/cabinet/equip_lists/" + env_no + "/sensor"), function (data) {
//  vm.$set('sensor_list', data.rows);
//});

//调控设备-start
//$.get(API("/env/environments/cabinet/equip_lists/" + env_no + "/controller"), function (data) {
