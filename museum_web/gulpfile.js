...
var dest_path = '../web/';//输出目录

lib.set('version', "2.2.05_P001");
lib.set('root', __dirname + "/src/");
lib.set('api_url', "/2.2.05_P001/base_api");//api地址,prod才启用,强制使用API大写进行替换
...
gulp.task('html', ['js'], function () {
    return gulp.src('./src/**/*.{vue,htm,html}')
        .pipe(lib.build())
        .pipe(gulp.dest(dest_path));
});
...
gulp.task('default', ['clean'], function () {
    gulp.start('js_libs', 'css', 'images', 'js', 'html','copy');
});
...
gulp.task('watch', function () {
    gulp.start('default');
    // 监听html,js
    gulp.watch('./src/**/*.{vue,htm,html,js}', function () {
        gulp.start('js', 'html');
    });
    // 监听css
